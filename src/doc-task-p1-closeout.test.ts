...
describe("§2.1 intent 透传 —— 失败通知不得打 applied 徽章", () => {
  ...
  it("正常答复带 output intent", async () => {
    const seen: Array<string | undefined> = [];
    const handler = createDocMentionHandler({
      botUid: "bot_1",
      dedupe: createMemoryDocMentionDedupeStore(),
      dispatch: async (_msg, _x, ctx) => {
        await ctx!.docTask!.postComment("改好了", undefined, "output");
        ctx!.docTask!.reportTurn!({
          finalDelivered: true,
          delivered: true,
          lost: false,
...
describe("§2.1 最后一跳 —— postHtmlDocReply 必须把 intent 映射成 status", () => {
  ...
  async function capturePostedStatus(intent?: "output" | "notice"): Promise<string | undefined> {
  ...
  it("output ⇒ applied", async () => {
    expect(await capturePostedStatus("output")).toBe("applied");
  });
  ...
  it("两条都失败 ⇒ 落一条 undelivered_after_ack", async () => {
    ...
    const handler = createDocMentionHandler({
      ...
      dispatch: async (_msg, _x, ctx) => {
        ...
        await ctx!.docTask!.postComment("改好了", undefined, "output").catch(() => {});
        return "completed";
      },
      // 所有 POST 都失败，兜底通知也一样
      postComment: async () => {
