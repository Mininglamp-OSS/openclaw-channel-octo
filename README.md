...
- A supporting Octo server binds each Bot Token to the first OpenClaw installation that registers it. The plugin keeps that installation ID under OpenClaw's canonical state directory at `octo/instance-id` (for example `$OPENCLAW_STATE_DIR/octo/instance-id`, `$OPENCLAW_HOME/.openclaw/octo/instance-id`, or the default `~/.openclaw/octo/instance-id`; legacy installations may resolve to `~/.clawdbot/octo/instance-id`). Persist this file in containers and do not copy it between installations. To intentionally move a Bot to another installation, rotate its Bot Token first. After a token has been bound, downgrading to a plugin version that does not send `instance_id` also requires token rotation before that older plugin can connect.
