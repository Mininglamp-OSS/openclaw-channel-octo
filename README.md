...
- `docTasks` (optional, default off): Master switch for document comment `@Bot` tasks. Strictly opt-in — it must be set to the boolean `true`, and while it is unset the plugin never starts the document event poller, so no document comment can reach an agent. Enabling it means any user who can comment on a document the Bot can read is able to direct that Bot, and the agent session runs with the Bot's own credentials — the Bot's document permissions are therefore the effective ceiling on what a commenter can cause. Slash commands are disabled on this path and comment text only ever enters the session as a quoted value, but the trust boundary is still "whoever can comment can task the Bot", so enable it only where that group is acceptable. Requires `docsApiUrl` (or a single-origin `apiUrl`) to be reachable, otherwise replies are silently lost as described above.
