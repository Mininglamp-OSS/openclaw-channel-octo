...
- A supporting Octo server binds each Bot Token to the first OpenClaw installation that registers it. The plugin keeps that installation ID at `$OPENCLAW_STATE_DIR/octo/instance-id` (or `$OPENCLAW_HOME/octo/instance-id`, default `~/.openclaw/octo/instance-id`). Persist this file in containers and do not copy it between installations. To intentionally move a Bot to another installation, rotate its Bot Token first.
