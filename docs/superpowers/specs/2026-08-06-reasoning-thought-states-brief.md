...
|---|---|---|
| 有内容 | 清洗后非空 | 原文(现状不变) |
| 有推理、无内容 | 文本等于宿主占位句(签名有、文本空) | **待定 A** |
| 被守卫拦下 | 触发点 4(必要时并入 2 / 3) | **待定 B**,必须与 A 可区分 |
| 无推理内容 | 触发点 1 | 不显示思考行,只留耗时(对齐扁平卡的 `💭 思考 · 4.7s`) |
| 结构性无思考步骤 | 触发点 5 / 6 | 不显示思考行 |

「被守卫拦下」是唯一能让人自查的信号 —— 看到它就知道该去查脱敏规则,而不是怀疑模型或路由。
...
- **A**:「有推理、无内容」显示什么。
- **B**:「被守卫拦下」显示什么 —— 需要既让运维看懂、又不暗示被隐藏内容的性质。
